...
const TripLibrary = ({ onBack, onViewTrip }) => {
  ...
  const fetchPublicTrips = useCallback(async () => {
    try {
      setLoading(true);
      const response = await getPublicTrips(1, 20, filters);
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <LoadingSpinner size={48} text="Loading Trip Library..." />
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-xl text-white/70 mb-8">
            Discover amazing itineraries shared by our community
          </p>
          
          <motion.button
            onClick={onBack}
            className="bg-white/10 text-white hover:bg-white/20 px-6 py-3 rounded-lg font-medium flex items-center space-x-2 mx-auto transition-colors"
        ...
        <motion.div
          ...
          initial="initial"
          animate="animate"
          className="bg-black rounded-2xl border border-white/10 p-6 mb-8"
        >
